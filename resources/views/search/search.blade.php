    ...
    function buildHTML(alldata) {
        ...
        alldata.forEach((listval) => {
            ...
            if (val != undefined) {
                ...
                html = html + '<div class=""><a href="' + view_vendor_details + '" class="check_vendor_' + val.id + '"><img alt="#" src="' + productStoreImage + '" onerror="this.onerror=null;this.src=\'' + placeholderImage + '\'" class="img-fluid item-img w-100"></a></div>';
                html = html + '</div>';
                html = html + '<div class="p-3 position-relative">';
                html = html + '<div class="list-card-body" ><h6 class="mb-1"><a href="' + view_vendor_details + '" class="text-black check_vendor_' + val.id + '">' + productStoreTitle + '</a></h6><p class="text-gray mb-3"><span class="fa fa-map-marker"></span> ' + val.location + '</p>' + ratinghtml + '</div>';
        ...
        $(".vendor_counts").text('Stores (' + vendorCount + ')');
        return html;
    }
    ...
    function buildProductHTML(allProductdata) {
        ...
        if (allProductdata != undefined && allProductdata != '') {
            ...
            allProductdata.forEach((listval) => {
                ...
                checkVendorExist(val.vendorID, val.id);
                var rating = 0;
                var reviewsCount = 0;
                if (val.hasOwnProperty('reviewsSum') && val.reviewsSum != 0 && val.hasOwnProperty('reviewsCount') && val.reviewsCount != 0) {
                    rating = (val.reviewsSum / val.reviewsCount);
                ...
                html = html + '<a href="' + view_product_details + '" class="check_vendor_' + val.vendorID + '"><img alt="#" src="' + photo + '" onerror="this.onerror=null;this.src=\'' + placeholderImage + '\'" class="img-fluid item-img w-100"></a></div><div class="py-2 position-relative"><div class="list-card-body position-relative"><h6 class="mb-1"><a href="' + view_product_details + '" class="arv-title check_vendor_' + val.vendorID + '">' + val.name + '</a></h6>';
        ...
        $(".products_counts").text('products (' + productCount + ')');
        return html;
    }

    async function checkVendorExist(vendorId, productId = '') {
            ...
            if (resultCheckVendor.docs.length > 0) {
                ...
                await database.collection('users').where('id', '==', vendorData.author).get().then(async function (resultCheckUser) {
                    if (resultCheckUser.docs.length > 0) {
                        ...
                        var view_product_details = "{{ route('productdetail',':id')}}";
                        view_product_details = view_product_details.replace(':id', productId);
                        if (productId == '') {
                            $('.check_vendor_' + vendorId).attr('href', view_vendor_details)
                        } else {
                            $('.check_vendor_' + vendorId).attr('href', view_product_details)
                        }
                    } else {
