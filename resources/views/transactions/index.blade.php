    ...
    <div class="container">
        <div class="transactions-banner p-4 rounded">
            <div class="row align-items-center text-center">
                <div class="col-md-6">
                    <h3 class="font-weight-bold h4 text-light">{{trans('lang.total')}} {{trans('lang.payment')}} : <span
                                id='total_payment'>></span></h3>
                </div>
            </div>
        </div>
        ...
        database.collection('users').where("id", "==", user_uuid).get().then(
            (amountsnapshot) => {
                ...
                amountsnapshot.docs.forEach((payment) => {
                    ...
                    if (currencyAtRight) {
                        jQuery("#total_payment").html(paymentDatas.wallet_amount.toFixed(decimal_degits) + currentCurrency);
                    } else {
                        jQuery("#total_payment").html(currentCurrency + paymentDatas.wallet_amount.toFixed(decimal_degits));
                    }
                });
            });
        jQuery("#overlay").show();
