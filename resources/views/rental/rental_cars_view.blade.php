...
<script type="text/javascript">
    ...
    var ratings = database.collection('items_review').where('driverId', "==", user_id);
            ...
            if (arrayCarImages.length > 0) {
                for (var i = 0; i < arrayCarImages.length; i++) {
                    carImagesHtml = '<li data-thumb="' + arrayCarImages[i] + '" class="lslide">' +
                        '                                        <img src="' + arrayCarImages[i] + '" onerror="this.onerror=null;this.src=\'' + placeholderImage + '\'"/>' +
                        '                                    </li>';
    ...
    async function getRentalVehicleTypeDetails(rentalCarsData) {
        ...
        var ref = rentalVehicleTypeUserRef.get().then(async function (snapshots) {
            ...
            var carRate = 0;
            if (rentalCarsData.hasOwnProperty('car_rate')) {
                carRate = rentalCarsData.car_rate;
                rentalCarRate = carRate;
            }
            if (isDriver && rentalCarsData.driver_rate) {
                carRate = parseInt(carRate) + parseInt(rentalCarsData.driver_rate);
                rentalDriverRate = rentalCarsData.driver_rate;
            }
            if (currencyAtRight) {
            ...
            var carDetailHtml = '       <div class="car-det-head mb-3">\n' +
                ...
                '                                        <h2>' + rentalCarsData.carName + ' ' + carMakes + '</h2>\n' +
                '                                        <div class="ratings">\n' +
