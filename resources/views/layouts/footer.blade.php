...
<script type="text/javascript">
                    ...
                    snapshots.docs.forEach((section) => {
                        ...
                        if (datas.serviceType == "On Demand Service") {
                            html =
                                ...
                                datas.color + '" service_type="' + datas.serviceType +
                                '" data-name="' + datas.name + '" data-dine_in="false" data-id="' +
                                datas.id + '">' +
                            ...
                            html =
                                '<div class="section-list-inner col-md-3 mb-4 select_section" data-color="' +
                                datas.color + '" service_type="' + datas.serviceType +
                                '" data-name="' + datas.name + '" data-dine_in="' + datas
                                .dine_in_active + '" data-id="' + datas.id + '">' +
                                '<div class="section-block bg-white rounded d-block py-3 px-2 text-center shadow-lg">' +
                                '<span class="section-img"><img alt="#" src="' + section_image +
                                '" onerror="this.onerror=null;this.src=\'' + placeholderImage +
                                '\'" class="img-fluid item-img w-100"></span>' +
        ...
        if ($("#section_lists").html() == '') {
            ...
            sectionsRef.get().then(async function(snapshots) {
                ...
                snapshots.docs.forEach((section) => {
                    ...
                    if (datas.serviceType == "On Demand Service") {
                        html =
                            '<div class="section-list-inner col-md-3 mb-4 select_section ' +
    ...
    async function sendOnDemandMailData(userEmail, userName, orderId, address, paymentMethod, product, quantity,
        couponCode, discount, taxSetting) {
        ...
        productDetailsHtml += '</td>';
        productDetailsHtml += '<td style="width: 20%; border: 1px solid rgb(0, 0, 0);">' + quantity +
            '</td><td style="width: 20%; border: 1px solid rgb(0, 0, 0);">' + price_item + priceUnit +
            '<td style="width: 20%; border: 1px solid rgb(0, 0, 0);">  ' + totalProductPrice + priceUnit + '</td>';
        productDetailsHtml += '</tr>';
        var totalAmount = 0;
    ...
    $(document).ready(function() {
        $(document).on("click", ".select_section", async function(e) {
            ...
            if (getCookie('service_type') == "Parcel Delivery Service" || getCookie(
                ...
                "Cab Service" ||
                getCookie('service_type') == "On Demand Service") {
                ...
                if (change.type == "modified") {
                    if (val.status == "Order Accepted") {
                        if (section_id == val.sectionId) {
                            ...
                            $("#order_notification_url").attr("href", routeondemand.replace(':id', val
                                .id));
                            $("#order_notification_modal").trigger("click");
                        }
                    ...
                    } else if (val.status == "Order Ongoing" && val.extraCharges == "" && val.endTime ==
                        null) {
                        if (section_id == val.sectionId) {
                    ...
                    } else if (val.status == 'Order Ongoing' && val.extraCharges != "" && val
                        .extraCharges != null && sessionStorage.getItem('extra_charge_notiifcation_' +
                            val.id) == null) {
                        if (section_id == val.sectionId) {
                    ...
                    } else if (val.status == "Order Ongoing" && val.endTime != null && sessionStorage
                        .getItem('stop_time_notiifcation_' + val.id) == null) {
                        if (section_id == val.sectionId) {
            ...
            snapshotslang.forEach((data) => {
                if (data.isActive == true) {
                    ...
                    $('#language_dropdown').append($("<option></option>").attr("value", data.slug)
                        .text(data.title));
                    $('#language_dropdown2').append($("<option></option>").attr("value", data.slug)
                        .text(data.title));
                }
            });
            if (langcount > 1) {
