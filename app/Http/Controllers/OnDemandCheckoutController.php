...
class OnDemandCheckoutController extends Controller
{
    /**
    ...
    public function proccesstopay()
    {
        ...
        if (@$ondemand_cart['cart_order']) {
            if ($ondemand_cart['cart_order']['payment_method'] == 'razorpay') {
                ...
                $amount = 0;
                return view('providersService.razorpay', ['is_checkout' => 1, 'cart' => $ondemand_cart, 'id' => $user->uuid, 'email' => $email, 'authorName' => $authorName, 'amount' => $total_pay, 'razorpaySecret' => $razorpaySecret, 'razorpayKey' => $razorpayKey, 'cart_order' => $ondemand_cart['cart_order']]);
                ...
                $payfast_return_url = route('ondemand-success');
                $payfast_notify_url = route('notify');
                $payfast_cancel_url = route('ondemand-pay');
                $authorName = $ondemand_cart['cart_order']['authorName'];
                $total_pay = $ondemand_cart['cart_order']['total_pay'];
                $amount = 0;
                $token = uniqid();
                ...
                $total_pay = $ondemand_cart['cart_order']['total_pay'];
                $amount = 0;
                return view('providersService.stripe', ['is_checkout' => 1, 'cart' => $ondemand_cart, 'id' => $user->uuid, 'email' => $email, 'authorName' => $authorName, 'amount' => $total_pay, 'stripeSecret' => $stripeSecret, 'stripeKey' => $stripeKey, 'cart_order' => $ondemand_cart['cart_order']]);
                ...
                $total_pay = $ondemand_cart['cart_order']['total_pay'];
                $amount = 0;
                return view('providersService.paypal', ['is_checkout' => 1, 'cart' => $ondemand_cart, 'id' => $user->uuid, 'email' => $email, 'authorName' => $authorName, 'amount' => $total_pay, 'paypalSecret' => $paypalSecret, 'paypalKey' => $paypalKey, 'cart_order' => $ondemand_cart['cart_order']]);
                ...
                $fields[] = $items;
                $item['items'] = $fields;
                $item['back_urls']['failure'] = route('ondemand-pay');
                $item['back_urls']['pending'] = route('notify');
                $item['back_urls']['success'] = route('ondemand-success');
            ...
            return redirect()->route('ondemand-checkout');
        }
    }
    ...
    public function razorpaypayment(Request $request)
    {
        ...
        Session::put('success', 'Payment successful');
        return redirect()->route('ondemand-success');
    }
    ...
    public function success()
    {
        ...
        return view('providersService.success', ['cart' => $ondemand_cart, 'id' => $user->uuid, 'email' => $email, 'payment_method' => $payment_method]);
    }
    ...
    public function orderComplete(Request $request)
    {
        ...
        $order_response = array('status' => true, 'order_complete' => true, 'html' => view('providersService.cart_item', ['ondemand_cart' => $cart, 'order_complete' => true, 'is_checkout' => 1])->render(), 'response' => $response);
